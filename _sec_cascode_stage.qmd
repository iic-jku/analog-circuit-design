...
We find that if $G_\mathrm{S} = 0$ (an open) then $g_\mathrm{out} = 0$, and if $G_\mathrm{S} = \infty$ (a short) then $g_\mathrm{out} = \gds$. We can calculate the benefits of a cascode if we assume we put a cascode on top of a common-source transitor stage (thus $G_\mathrm{S} = \gds'$) and get
$$
...
To the calculate the input impedance of a cascode (i.e., looking into the source) we replace $G_\mathrm{S}$ with a current source. The resulting small-signal equivalent circuit is shown in @fig-cascode-small-signal-simplified-in. 
...
However, setting $G_\mathrm{D} = \infty$ (a short or low-ohmic impedance) results in the well-known result of $g_\mathrm{in} = \gm + \gds \approx \gm$, which means that the input impedance looking into a cascode is approximately $1/\gm$. This has the practical benefit that a capacitance connected at this noise results in a high-frequency pole, which is often not critical in terms of stability. Further, the voltage swing at a cascode input node is small due to the often small impedance, and this minimizes the Miller effect at connected inter-node capacitors (see @sec-miller-theorem).
