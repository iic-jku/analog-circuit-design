...
If we formulate the drain current mismatch standard deviation $\sigma(\Delta \ID / \ID)$ of two nominally identical MOSFET we get

$$
\sigma \left( \frac{\Delta \ID}{\ID} \right) = \frac{A_\mathrm{mosfet}}{\sqrt{W L}}
$$ {#eq-mosfet-mismatch}

with $A_\mathrm{mosfet}$ being a mismatch parameter for a given CMOS technology and $\sigma (\cdot)$ being the standard deviation of the parameter in parentheses.
...
$$
\sigma (\Delta \Vth) = \frac{A_\mathrm{vth}}{\sqrt{W L}}
...
Similar to the MOSFET mismatch discussed in @sec-mosfet-mismatch, resistors will also show mismatch. Equivalently to @eq-mosfet-mismatch the standard deviation of the resistor mismatch $\sigma(\Delta R / R)$ can be characterized by
...
Similar to the resistor matching discussed in @sec-resistor-mismatch, capacitors will also show mismatch. Equivalently to @eq-mosfet-mismatch the standard deviation of the capacitor mismatch $\sigma(\Delta C / C)$ can be characterized by
