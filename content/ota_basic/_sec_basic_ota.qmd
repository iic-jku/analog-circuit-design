...
This leaves plenty of margin for additional slow-signal settling due to the limited bandwidth, as well as for reducing the supply current.
...
$$
A(s) = \frac{V_\mathrm{out}(s)}{V_\mathrm{in}(s)} = \frac{\gm[1,2]}{2} \frac{2 \cdot \gm[34] + 2 \cdot s \Cgs[3,4]}{(\gm[3,4] + 2 \cdot s \Cgs[3,4]) (\gds[2] + \gds[4] + s C_\mathrm{load})}.
$$ {#eq-simple-ota-gain}
...
## Component Mismatch {#sec-mimatch}
