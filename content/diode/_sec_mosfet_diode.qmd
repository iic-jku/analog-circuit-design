...
1. The dc gain $\gm / \gds$ is the self-gain of the MOSFET, so $20 \log_{10}(0.2 \cdot 10^{-3} / 9.5 \cdot 10 ^{-6}) = 26.5\,\text{dB}$.
2. There is a pole at $s_\mathrm{p} = -\gds / \Cgs$, i.e. at a frequency of $f_\mathrm{p} = 9.5 \cdot 10 ^{-6} / (2 \pi \cdot 1.4 \cdot 10^{-15}) = 1.1\,\text{GHz}$.
...
From simulation we see that the open-loop gain is $24.9\,\text{dB}$ at low frequencies, which matches quite well our prediction of $26.5\,\text{dB}$. In the Bode plot we see a low-pass with a $-3\,\text{dB}$ corner frequency of $1.4\,\text{GHz}$, which again is fairly close to our prediction of $1.1\,\text{GHz}$. 
...
$$
\sqrt{V_\mathrm{n,rms}^2} = \sqrt{\frac{1.38 \cdot 10^{-23} \cdot 300 \cdot 0.84}{1.4 \cdot 10^{-15}}} = 1.58\,\text{mV},
$$
