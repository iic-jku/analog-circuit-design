...
2. Using the self-gain plot, we see that $\gm/\gds \approx 18.4$, so $\gds \approx 10.9\,\mu\text{S}$. The $f_\mathrm{T}$ can easily be found in the respective plot to be $f_\mathrm{T} = 23\,\text{GHz}$. Note that the self-gain has to be read at $\VDS = \VGS \approx 0.59\,\text{V}$, **not** at the $\VDS = \VDD/2$ used for the characterization plots in @sec-techsweep-nmos: in a MOSFET diode the drain is tied to the gate, and $\gds$ is a strong function of $\VDS$. The linked notebook does this self-consistently.
...
4. Since we are looking at the graphs, we further find $\gamma = 0.81$, $V_\mathrm{ds,sat} = 0.18\,\text{V}$, and $f_\mathrm{co} \approx 16\,\text{MHz}$.
5. In addition, we expect $\VGS \approx 0.6\,\text{V}$.

You can find an example Jupyter notebook to accurately extract these values [here](./gmid/sizing_mosfet_diode.ipynb). An Xschem schematic for this exercise is provided [as well](./xschem/mosfet_diode_sizing.sch).
:::
...
1. The dc gain $\gm / \gds$ is the self-gain of the MOSFET, so $20 \log_{10}(0.2 \cdot 10^{-3} / 10.9 \cdot 10 ^{-6}) = 25.3\,\text{dB}$.
2. There is a pole at $s_\mathrm{p} = -\gds / \Cgs$, i.e. at a frequency of $f_\mathrm{p} = 10.9 \cdot 10 ^{-6} / (2 \pi \cdot 1.4 \cdot 10^{-15}) = 1.25\,\text{GHz}$.

With this single pole location in $T(s)$ this loop is perfectly stable under all conditions (remember that a single pole results in a maximum phase shift of --90$^\circ$).
...
From simulation we see that the open-loop gain is $24.9\,\text{dB}$ at low frequencies, which matches our prediction of $25.3\,\text{dB}$ very well. In the Bode plot we see a low-pass with a $-3\,\text{dB}$ corner frequency of $1.4\,\text{GHz}$, which again is close to our prediction of $1.25\,\text{GHz}$. 
