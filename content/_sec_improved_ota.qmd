...
The current mirrors constructed out of $M_\mathrm{5/5C,6/6C}$ and $M_\mathrm{3/3C,4/4C}$ are a special kind of **cascode current mirror for low-voltage operation**, also referred to as high-swing cascode current mirror [@Jespers_Murmann_2017]. This type is very often used, as it forces the $\VGS$ and $\VDS$ of $M_{5,6}$ (and $M_{3,4}$) to be equal, so the current mirror ratio is independent of $\gds$. 

::: {.callout-tip title="Exercise: Cascode Current Mirror vs. High-Swing Cascode Current Mirror"}
Try to verify the above statement of equal drain-source voltages by deriving both, an equation for $\VDS[5]$ assuming a high-swing cascode current mirror (@fig-improved-ota) and $\VDS[5]$ in case of a simple cascode current mirror, where the reference branch $(M_\mathrm{6/6C})$ is comprised of two mosfet diodes.
:::
...
8. Sensitive bias nodes are buffered with decoupling capacitors. We are using MOSFET as nonlinear capacitors, which is not an issue in this application, but we value the increased capacitive density. Please note how the MOSFET are connected (to $\VDD$? or to $\VSS$?).
