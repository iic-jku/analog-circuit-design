...
- A variation of the critical dimensions of the MOSFET ($W/L$).
...
with $A_\mathrm{mosfet}$ being a mismatch parameter for given CMOS technology.
...
$$
V_\mathrm{offs}^2 = \left( \frac{\VGS - \Vth}{2} \frac{\Delta (W/L)}{{(W/L)}} \right)^2 + \Delta \Vth^2.
$$ {#eq-diffpair-offset}
...
How can we now cope with mismatch in design and simulation? We can account the transitor mismatch according to @eq-mosfet-mismatch in circuit analysis and quantify its effect. We can then take this into account for circuit sizing.

We can also simulate the effects of MOSFET mismatch on circuit performance by doing a [**Monte Carlo**](https://en.wikipedia.org/wiki/Monte_Carlo_method) simulation. Here, a random realization of the variations of all circuit components (where mismatch is modelled) is taken and the circuit is simulated. Then, another random realization is simulated, and so on. In summary, we run the same simulation for different realization $N$ times and evaluate the resulting variations of the circuit parameters. If $N$ is large enough then the distributions should approach a [Gaussian distribution](https://en.wikipedia.org/wiki/Normal_distribution), and we can then estimate the variances of the circuit parameters. Using statistical analysis we can then assess the **yield** of a circuit, i.e., how many circuit realizations will meet the specification.

::: {.callout-note title="Number of Monte Carlo Simulation Runs"}
The number of Monte Carlo simulation runs $N$ has to be large enough to approach Gaussian distributions to estimate the variances. However, this comes at the price of a large simulation time, so a balance has to be found. Often, $N = 250$ is a good compromise between simulation time and well-behaved parameter distributions.
:::
...
Similar to the MOSFET mismatch discussed in @sec-mosfet-mismatch also resistors will show mismatch. Similar to @eq-mosfet-mismatch the resistor mismatch $\Delta R / R$ can be characterized by

$$
\frac{\Delta R}{R} = \frac{A_\mathrm{res}}{\sqrt{W L}}
$$ {#eq-resistor-mismatch}

with $A_\mathrm{res}$ being a mismatch parameter for given CMOS technology.

With @eq-resistor-mismatch we now have two criteria how to select the width $W$ of a specific resistor (the length $L$ is then derived from the required resistance value with $R = R_\square \cdot L / W$):

1. Current handling capability (the larger the $W$, the more dc current a resistor can carry)
2. Resistor mismatch (the larger the $W$, the larger the $W L$ for a given $L/W$)

If a resistor's dimensions are not limited by the two criteria above then we usually choose the minimum width that is allowed for a given resistor in a specific technology (for SG13G2 it is $L_\mathrm{min} = 0.5\,\mu\text{m}$). 

## 5T-OTA Simulation versus PVT {#sec-basic-ota-simulation-pvt}
...
Looking at the CACE report in @nte-basic-ota-cace-result we see that the output voltage specification is not met due to MOSFET mismatch. We have not considered transistor mismatch in the circuit sizing procedure, and as a result the selected MOSFET dimensions proof to be too small. We should now go back and increase the transistor sizing (increase $L$ significantly while keeping the $\gmid$ values). Likely we will find that some performance parameter now deteriorated due to the increased MOSFET dimensions, and we need to iterate until all performance metrics are met in the presence of transistor mismatch.

::: {.callout-tip title="Exercise: Re-Sizing of 5T-OTA for Mismatch"}
Go back to @sec-basic-ota-sizing and repeat the sizing procedure of the 5T-OTA by increasing the $L$ of the MOSFETs significantly. Focus first on the differential pair as it will likely have the biggest impact. Then, tune the size of the output current mirror is necessary.
