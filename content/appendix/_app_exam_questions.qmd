...
- **Output conductance** $\gds = {\partial \ID}/{\partial \VDS} = r_\mathrm{out}^{-1}$: Describes how the drain current varies with drain-source voltage (ideally zero in saturation, but finite due to channel-length modulation). It limits the maximum achievable voltage gain. In the triode region, $\gds$ is the conductance of the channel (i.e., the channel resistance is $R_\mathrm{on} = 1/\gds$), which can be electrically controlled by $\VGS$.
:::
...
This gives $\gmid \approx 26-32\,\text{V}^{-1}$ as the maximum (for the LV NMOS of SG13G2 the tables yield about $28\,\text{V}^{-1}$, i.e. $n \approx 1.38$).
...
- Due to source degeneration: $G_\mathrm{m,cm} = \frac{\gm}{1 + 2 \cdot \gm \cdot r_\mathrm{tail}} \approx \frac{1}{2 \cdot r_\mathrm{tail}}$

**Common-mode rejection ratio (CMRR):**
$$\text{CMRR} = \frac{G_\mathrm{m,dm}}{G_\mathrm{m,cm}} = 2 \cdot \gm \cdot r_\mathrm{tail}$$
:::
...
$$\sigma\{V_\mathrm{os}\} = \sqrt{\sigma^2\{\Delta\Vth\} + \left(\frac{\VGS - \Vth}{2}\right)^2 \cdot \sigma^2\left\{\frac{\Delta\gm}{\gm}\right\}}$$

Or equivalently:
$$\sigma\{V_\mathrm{os}\} = \sqrt{\sigma^2\{\Delta\Vth\} + \left(\frac{V_{od}}{2}\right)^2 \cdot \sigma^2\left\{\frac{\Delta(W/L)}{(W/L)}\right\}}$$

Note that the offset is a *random* quantity, so what these expressions give is its standard deviation, not a single deterministic value.
...
$$V_\mathrm{n,out,rms}^2 = \frac{kT}{C_\mathrm{L}} \left( 2\gamma_{1,2} + 2\gamma_{3,4} \frac{\gm[3,4]}{\gm[1,2]} \right)$$

**Analysis:**

1. Input pair ($M_1$, $M_2$): Each contributes $\gamma_{1,2}$, total factor $2 \gamma_{1,2}$
2. Load pair ($M_3$, $M_4$): Each contributes $\gamma_{3,4} \cdot \gm[3,4]/\gm[1,2]$ when referred to the input
...
- GBW = 10 MHz with $C_\mathrm{L} = 5\,\text{pF}$
- DC gain > 50 dB
...
$$\gm = 2\pi \cdot \text{GBW} \cdot C_\mathrm{L} = 2\pi \cdot 10\,\text{MHz} \cdot 5\,\text{pF} = 314\,\mu\text{S}$$

**Step 2: Choose $\gmid$ and calculate current**

For a moderate speed/power trade-off, choose $\gmid = 15\,\text{V}^{-1}$:
$$\ID = \frac{\gm}{\gmid} = \frac{314\,\mu\text{S}}{15\,\text{V}^{-1}} = 21\,\mu\text{A}$$

Total tail current: $I_\mathrm{tail} = 2 \times 21\,\mu\text{A} = 42\,\mu\text{A}$

Power: $P = \VDD \cdot I_\mathrm{tail} = 1.5\,\text{V} \times 42\,\mu\text{A} = 63\,\mu\text{W}$
...
Need $\gmgds > 632$. From lookup tables at $\gmid = 15$, $L = 1\,\mu\text{m}$: $\gmgds \approx 38$

**Not sufficient!** Options:

- Increase L to 10 µm (increases $\gmgds$ to ~75, still not sufficient)
- Use cascode topology
- Accept lower gain --- this we will do for this simple design

**Step 4: Size transistors**

Using lookup tables: $\ID/W$ at $\gmid = 15$, $L = 10\,\mu\text{m}$ ≈ 0.2 µA/µm

$$W_{1,2} = \frac{\ID}{\ID/W} = \frac{21\,\mu\text{A}}{0.2\,\mu\text{A}/\mu\text{m}} = 105\,\mu\text{m}$$

We assume for this simple design that the current mirror load ($M_3$, $M_4$) is sized to have $\gm[3,4] \approx \gm[1,2]$, so $W_{3,4} = 105\,\mu\text{m}$ as well.
:::

::: {.callout-note title="Question 12.2 --- Current Mirror Design" collapse="true"}
**Question:** Design a cascode current mirror to provide $I_\mathrm{out} = 50\,\mu\text{A}$ from $I_\mathrm{ref} = 10\,\mu\text{A}$. Minimize the output compliance voltage while achieving $r_\mathrm{out} > 5\,\text{M}\Omega$.

**Solution:**

**Current ratio:** $I_\mathrm{out}/I_\mathrm{ref} = 5$

**Step 1: Choose $\gmid$ for minimum $V_\mathrm{ds,sat}$**

High $\gmid = 18\,\text{V}^{-1}$: $V_\mathrm{ds,sat} \approx 2/18 = 111\,\text{mV}$

**Step 2: Calculate required $\gmgds$**

For simple current mirror: $r_\mathrm{out} = 1/\gds$

Required: $r_\mathrm{out} > 5\,\text{M}\Omega$ at $\ID = 50\,\mu\text{A}$ → $\gds < 200\,\text{nS}$

At $\gmid = 18$: $\gm = 18 \times 50\,\mu\text{A} = 900\,\mu\text{S}$ → $\gmgds > 900\,\mu\text{S} / 200\,\text{nS} = 4500$

This is too high for a single device → need cascode!

**Step 3: Cascode design**

With cascode: $r_\mathrm{out} \approx \gm[,casc] \cdot r_\mathrm{ds,casc} \cdot r_\mathrm{ds,main} \approx (\gmgds)^2 / \gm$

For $\gmgds = 75$ (achievable with L = 10 µm) at $\gm = 900\,\mu\text{S}$: $r_\mathrm{out} = 75^2 / 900\,\mu\text{S} = 5625 / 0.0009\,\text{S} \approx 6.25\,\text{M}\Omega$

This meets the requirement of $r_\mathrm{out} > 5\,\text{M}\Omega$

**Conclusion:** A standard cascode current mirror with $L \approx 10\,\mu\text{m}$ achieving $\gmgds \approx 75$ is sufficient.

**Step 4: Minimum output voltage (using high-swing current mirror)**

$V_\mathrm{out,min} = 2 \times V_\mathrm{ds,sat} \approx 2 \times 111\,\text{mV} = 222\,\text{mV}$
:::

::: {.callout-note title="Question 12.3 --- Common-Source Amplifier" collapse="true"}
**Question:** Design a common-source amplifier for a 50 Ω measurement system with voltage gain $|A_\mathrm{v}| = 10$ (20 dB). The amplifier should be matched to 50 Ω at input and output.

**Solution:**

**Matching conditions:**

- Input: the gate of the MOSFET is essentially capacitive, so the $50\,\Omega$ input match has to be provided by a shunt resistor $R_\mathrm{sh} = 50\,\Omega$ from the gate to ground
- Output: the drain load resistor is set to $R_\mathrm{D} = 50\,\Omega$ so that the amplifier presents a matched source impedance; together with the $50\,\Omega$ load this gives $R_\mathrm{load,eff} = R_\mathrm{D} \parallel R_\mathrm{L} = 25\,\Omega$

**Voltage gain** (from the gate to the load):
$$A_\mathrm{v} = -\gm \cdot R_\mathrm{load,eff} = -\gm \times 25\,\Omega$$
...
**Alternative approach:** Use a common-source stage followed by a source follower.
:::
...
1. Input stage noise is amplified by the full gain of the stage, so it appears at the output essentially undiminished when referred back to the input
2. The noise of the load devices, referred back to the input, is scaled by the ratio of transconductances $\gm[3,4]/\gm[1,2]$ --- see @eq-basic-ota-output-noise
3. Since we design for $\gm[3,4] < \gm[1,2]$ anyway (to minimize thermal noise), the load contribution is suppressed by that same ratio
4. For good noise performance: Large input devices (more area → less flicker noise); the load devices can be smaller, but only within the limits set by the $\gm[3,4]/\gm[1,2]$ ratio
:::
