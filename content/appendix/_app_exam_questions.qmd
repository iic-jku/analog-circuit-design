...
**Question:** Define the three main small-signal parameters of a MOSFET ($\gm$, $\gds$, $\gmb$) and explain their physical meaning.
...
- **Output conductance** $\gds = {\partial \ID}/{\partial \VDS} = r_\mathrm{out}^{-1}$: Describes how the drain current varies with drain-source voltage in saturation (ideally zero, but finite due to channel-length modulation). It limits the maximum achievable voltage gain.

- **Bulk transconductance** $\gmb = {\partial \ID}/{\partial \VSB}$: Describes how the drain current changes with source-bulk voltage due to the body effect. Typically $\gmb \approx 0.1 \cdot \gm$ to $0.3 \cdot \gm$.
:::
...
- **Triode region:** $\VGS > \Vth$ and $\VDS < \VGS - \Vth$ (or $\VDS < V_\mathrm{ds,sat}$)
- **Saturation region:** $\VGS > \Vth$ and $\VDS \geq \VGS - \Vth$ (or $\VDS \geq V_\mathrm{ds,sat}$)
...
3. High intrinsic gain ($\gmgds$) can be achieved
4. Linear relationship between small-signal input voltage and output current
...
**Question:** Define the transit frequency $f_T$ of a MOSFET. How does it relate to the transistor's speed capability?
...
**Question:** Derive the theoretical limits of $\gmid$ in weak inversion and strong inversion. What is the typical range of $\gmid$ used in practical analog design?
...
| Parameter | High $\gmid$ (>15) | Low $\gmid$ (<10) |
|-----------|-------------------|-------------------|
...
| Noise (thermal) | Same $\gm$ → same noise | Same |
| Matching | Generally better | Depends on area |
...
For a MOSFET with gate shorted to drain, applying KCL at the drain node:
$$i_{test} = \gm \vgs + \gds \vds + s\Cgs \vgs$$

Since $\vgs = \vds = v_{test}$:
$$i_{test} = (\gm + \gds + s\Cgs) v_{test}$$

Therefore:
$$Z_{diode}(s) = \frac{v_{test}}{i_{test}} = \frac{1}{\gm + \gds + s\Cgs}$$

**At DC:** $Z_{diode} \approx \frac{1}{\gm}$ (since $\gm \gg \gds$)

**Corner frequency:** $f_c = \frac{\gm + \gds}{2\pi \Cgs} \approx \frac{\gm}{2\pi \Cgs} \approx f_T$

The impedance is resistive ($\approx 1/\gm$) at low frequencies and becomes capacitive at high frequencies.
:::
...
The noise-bandwidth product remains constant, which is a fundamental trade-off.
:::
...
**Question:** For a basic NMOS current mirror with input transistor M1 and output transistor M2, derive the output current ratio. What are the main sources of error in a practical current mirror?
...
1. **Channel-length modulation:** Different $\VDS$ on M1 and M2 causes current mismatch
$$\frac{\Delta I}{I} \approx \frac{\Delta \VDS}{\VDS + 1/\lambda} = \frac{\Delta \VDS \cdot \gds}{\ID}$$

2. **Threshold voltage mismatch:** Random $\Delta\Vth$ variations
$$\frac{\Delta I}{I} \approx \gm \cdot \frac{\Delta \Vth}{\ID} = \gmid \cdot \Delta \Vth$$

3. **W/L mismatch:** Lithographic variations in device dimensions

4. **Temperature gradients:** Different operating temperatures across the chip

5. **Finite output resistance:** Limits the current source quality ($r_{out} = 1/\gds$)
:::
...
**Simple current mirror:** $r_{out} = r_{ds} = \frac{1}{\gds}$

**Improvements:**

1. **Cascode current mirror:**
$$r_{out} = r_{ds,casc} + r_{ds}(1 + \gm[,casc] \cdot r_{ds,casc}) \approx \gm[,casc] \cdot r_{ds,casc} \cdot r_{ds}$$

2. **Source degeneration:**
$$r_{out} = r_{ds} + R_{deg}(1 + \gm \cdot r_{ds}) \approx \gm \cdot r_{ds} \cdot R_{deg}$$

3. **Regulated cascode:**
$$r_{out} \approx r_{ds} \cdot \gm[,casc] \cdot r_{ds,casc} \cdot \gm[,aux] \cdot r_{ds,aux}$$

The improvement factor for cascoding is approximately $\gmgds$ (intrinsic gain).
:::
...
In a high-swing cascode, the bias voltage $V_{bias}$ for the cascode transistor is chosen such that both the main transistor and the cascode transistor operate just at the edge of saturation:

$$V_{bias} = \VGS[,casc] + V_{ds,sat,main}$$

**Minimum output voltage:**
$$V_{out,min} = V_{ds,sat,casc} + V_{ds,sat,main} = 2 \cdot V_{ds,sat}$$

For typical $\gmid = 15\,\text{V}^{-1}$: $V_{out,min} \approx 2 \times 133\,\text{mV} \approx 270\,\text{mV}$

This is significantly lower than a standard cascode where $V_{out,min} = \VGS + V_{ds,sat}$ (typically >500 mV).
:::
...
**Question:** For an ideal differential pair with tail current source $I_{tail}$, derive the differential-mode and common-mode transconductance.

**Solution:**

**Differential mode:** With $v_{in,dm} = v_{inp} - v_{inn}$ applied:

- The tail node acts as a virtual ground
- Each transistor sees $v_{gs} = v_{in,dm}/2$
- $i_{out} = \gm \cdot v_{in,dm}/2 - (-\gm \cdot v_{in,dm}/2) = \gm \cdot v_{in,dm}$

$$G_{m,dm} = \gm$$

**Common mode:** With $v_{in,cm}$ applied to both inputs:

- The tail current source presents impedance $r_{tail} = 1/\gds[,tail]$
- Due to source degeneration: $G_{m,cm} = \frac{\gm}{1 + 2\gm \cdot r_{tail}} \approx \frac{1}{2 \cdot r_{tail}}$

**Common-mode rejection ratio (CMRR):**
$$CMRR = \frac{G_{m,dm}}{G_{m,cm}} = \gm \cdot (2 \cdot r_{tail}) = 2 \cdot \gm \cdot r_{tail}$$
:::
...
$$V_{os} = \sqrt{\sigma^2(\Delta\Vth) + \left(\frac{\VGS - \Vth}{2}\right)^2 \cdot \sigma^2\left(\frac{\Delta\gm}{\gm}\right)}$$

Or equivalently:
$$V_{os} = \sqrt{\sigma^2(\Delta\Vth) + \left(\frac{V_{od}}{2}\right)^2 \cdot \sigma^2\left(\frac{\Delta(W/L)}{(W/L)}\right)}$$
...
**Question:** Derive the DC voltage gain of a basic 5-transistor OTA with differential pair (M1, M2), current mirror load (M3, M4), and tail current source (M5).

**Solution:**

The small-signal analysis yields:

$$A_0 = \frac{v_{out}}{v_{in,dm}} = \frac{\gm[12]}{\gds[2] + \gds[4]}$$

where:

- $\gm[12]$ is the transconductance of the input transistors
...
$$A_0 = \frac{\gm[12]}{\gds[2]} \cdot \frac{1}{1 + \gds[4]/\gds[2]} = \frac{(\gmgds)_{12}}{1 + \gds[4]/\gds[2]}$$

For equal output conductances: $A_0 \approx \frac{(\gmgds)_{12}}{2}$
:::

::: {.callout-note title="Question 6.2 --- Bandwidth and GBW" collapse="true"}
**Question:** For a 5T-OTA driving a load capacitance $C_L$, derive the dominant pole frequency and the gain-bandwidth product (GBW).

**Solution:**

**Dominant pole:** The output node has total capacitance $C_L$ (assuming $C_L \gg$ parasitic capacitances) and total conductance $\gds[2] + \gds[4]$:

$$f_p = \frac{\gds[2] + \gds[4]}{2\pi C_L}$$

**Gain-bandwidth product:**
$$GBW = A_0 \cdot f_p = \frac{\gm[12]}{\gds[2] + \gds[4]} \cdot \frac{\gds[2] + \gds[4]}{2\pi C_L} = \frac{\gm[12]}{2\pi C_L}$$

Note that GBW is independent of the output conductances and depends only on $\gm$ and $C_L$.

**Design implication:** For a given GBW requirement and load capacitance:
$$\gm[12] = 2\pi \cdot GBW \cdot C_L$$
:::
...
$$\frac{V_{out}}{V_{in}} = \frac{316}{317} = 0.9968$$
...
With cascode transistors (M2C, M4C) added:

$$A_0 = \frac{\gm[12]}{g_{out,n} + g_{out,p}}$$

where:
$$g_{out,n} = \gds[2] \cdot \frac{\gds[2C]}{\gm[2C]} \quad \text{and} \quad g_{out,p} = \gds[4] \cdot \frac{\gds[4C]}{\gm[4C]}$$

Therefore:
$$A_0 \approx \frac{\gm[12]}{\gds[2] \cdot \frac{\gds[2C]}{\gm[2C]} + \gds[4] \cdot \frac{\gds[4C]}{\gm[4C]}}$$
...
$$V_{out,max} = \VDD - V_{ds,sat,M3} - V_{ds,sat,M4C}$$
$$V_{out,min} = V_{ds,sat,M5} + V_{ds,sat,M2} + V_{ds,sat,M2C}$$

Total swing: $\VDD - 4 \cdot V_{ds,sat}$

**Folded-cascode OTA output swing:**
$$V_{out,max} = \VDD - 2 \cdot V_{ds,sat}$$
$$V_{out,min} = 2 \cdot V_{ds,sat}$$

Total swing: $\VDD - 4 \cdot V_{ds,sat}$ (similar)

**Key difference:** The folded-cascode allows rail-to-rail input common-mode range, while telescopic has limited input CM range.
:::
...
**Question:** Derive the output impedance looking into the drain of a cascode transistor, assuming the source is connected to a current source with output resistance $r_{ds,CS}$.

**Solution:**

Using small-signal analysis with test voltage $v_t$ at the drain:

$$g_{out} = \frac{i_t}{v_t} = \frac{\gds \cdot G_S}{G_S + \gm + \gds}$$

where $G_S = 1/r_{ds,CS} = \gds[,CS]$ is the source conductance.

For $\gm \gg \gds$ and $\gm \gg G_S$:
$$g_{out} \approx \frac{\gds \cdot G_S}{\gm} = \gds[,CS] \cdot \frac{\gds}{\gm}$$

Therefore:
$$r_{out} = \frac{1}{g_{out}} = r_{ds,CS} \cdot \frac{\gm}{\gds} = r_{ds,CS} \cdot (\gmgds)$$

**Conclusion:** The cascode increases output impedance by a factor equal to the intrinsic gain ($\gmgds$).
:::
...
With drain shorted to AC ground ($G_D \to \infty$):

$$g_{in} = \frac{(\gm + \gds) \cdot G_D}{\gds + G_D} \approx \gm + \gds \approx \gm$$

Therefore: $Z_{in} \approx 1/\gm$
...
A bandgap reference combines two voltages with opposite temperature coefficients:

1. **$\VBE$ (CTAT):** $\frac{\partial \VBE}{\partial T} \approx -2\,\text{mV/K}$

2. **$\Delta\VBE$ (PTAT):** $\Delta\VBE = \frac{kT}{q}\ln\left(\frac{J_1}{J_2}\right)$, where $\frac{\partial(\Delta\VBE)}{\partial T} = \frac{k}{q}\ln\left(\frac{J_1}{J_2}\right) > 0$

**Reference voltage:**
$$V_{ref} = \VBE + K \cdot \Delta\VBE$$

where $K$ is chosen such that the temperature coefficients cancel:
$$\frac{\partial V_{ref}}{\partial T} = \frac{\partial \VBE}{\partial T} + K \cdot \frac{\partial(\Delta\VBE)}{\partial T} = 0$$

At the zero-TC point, $V_{ref}$ equals the extrapolated bandgap voltage of silicon at 0 K:
$$V_{ref} = V_{g0} \approx 1.205\,\text{V}$$
:::
...
$$I_{PTAT} = \frac{\Delta\VBE}{R} = \frac{kT}{qR}\ln\left(\frac{J_1}{J_2}\right) = \frac{kT}{qR}\ln(N)$$

where $N = J_1/J_2$ is the current density ratio (often implemented by area ratio).

**Temperature coefficient:**
$$\frac{\partial I_{PTAT}}{\partial T} = \frac{k \ln(N)}{qR}$$

$$\frac{1}{I_{PTAT}}\frac{\partial I_{PTAT}}{\partial T} = \frac{1}{T} \approx +3300\,\text{ppm/K at 300 K}$$
...
**Question:** Write the expression for thermal noise current of a MOSFET in saturation. What is the noise excess factor $\gamma$?

**Solution:**

**Thermal noise current spectral density:**
$$\overline{i_n^2} = 4kT\gamma g_{d0}\,\text{[A}^2\text{/Hz]}$$
...
- $g_{d0}$ is the zero-bias drain conductance ($\approx \gm$ in saturation)
...
$$\overline{v_{n,in}^2} = \frac{\overline{i_n^2}}{\gm^2} = \frac{4kT\gamma}{\gm}$$
:::

::: {.callout-note title="Question 10.2 --- OTA Output Noise" collapse="true"}
**Question:** Derive the total output noise of a 5T-OTA in a unity-gain buffer configuration with load capacitance $C_L$.

**Solution:**

The integrated output noise (kT/C noise):
$$V_{n,out,rms}^2 = \frac{kT}{C_L}\left(2\gamma_{12} + 2\gamma_{34}\left(\frac{\gm[34]}{\gm[12]}\right)^2\right)$$

**Analysis:**

1. Input pair (M1, M2): Each contributes $\gamma_{12}$, total factor $2\gamma_{12}$
2. Load pair (M3, M4): Each contributes $\gamma_{34} \cdot (\gm[34]/\gm[12])^2$ when referred to the input

**To minimize noise:**

- Increase $C_L$ (but reduces bandwidth)
- Minimize $\gm[34]/\gm[12]$ ratio (keep load $\gm$ low)
...
**Question:** State Pelgrom's law for MOSFET threshold voltage mismatch. How should transistors be sized to achieve 1 mV offset in a differential pair?

**Solution:**

**Pelgrom's law:**
$$\sigma(\Delta\Vth) = \frac{A_{vth}}{\sqrt{WL}}$$

where $A_{vth}$ is a technology-dependent constant (typical: 3--10 mV·µm for modern CMOS).

**For 1 mV offset:**
Assuming $A_{vth} = 5\,\text{mV}\cdot\mu\text{m}$:
...
$r_{out} = 100^2 / 900\,\mu\text{S} \approx 11\,\text{k}\Omega$

This is far too low! Need much longer L. With L = 5 µm for $\gmgds \approx 500$:
$r_{out} \approx 500^2 / 900\,\mu\text{S} \approx 280\,\text{k}\Omega$ --- still insufficient for 10 MΩ.

**Conclusion:** A regulated cascode or gain-boosted topology is required to achieve $r_{out} > 10\,\text{M}\Omega$.
...
**Question:** Why does the GBW of an OTA depend only on $\gm$ and $C_L$, not on the output resistance?

**Solution:** 
$$GBW = A_0 \cdot f_p = \frac{\gm}{g_{out}} \cdot \frac{g_{out}}{2\pi C_L} = \frac{\gm}{2\pi C_L}$$

The output conductance $g_{out}$ appears in both the gain ($A_0 = \gm/g_{out}$) and the pole frequency ($f_p = g_{out}/(2\pi C_L)$), so it cancels out. This is a fundamental property of single-pole systems.
:::
...
1. **Wider input common-mode range:** Input pair can swing from rail to rail
2. **Decoupled input and output DC levels:** More flexibility in biasing
3. **Same output swing:** Both have $\VDD - 4V_{ds,sat}$
...
1. Simple cascode: $r_{out} \approx \gm \cdot r_{ds,casc} \cdot r_{ds,main}$
2. Regulated cascode: $r_{out} \approx A_{aux} \cdot \gm \cdot r_{ds,casc} \cdot r_{ds,main}$

The auxiliary amplifier gain $A_{aux}$ provides an additional multiplication factor, dramatically increasing output impedance. This is essentially applying negative feedback to stabilize the drain voltage.
:::
