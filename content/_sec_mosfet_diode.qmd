...
Exemplary implementations of power-down switches are shown in the Xschem implementation of the improved OTA in @fig-improved-ota-design. What $W/L$ ratio should these transistors have? Well, in general, switches often have minimum length to be fast. However, if there are no further specifications like power-down time, then these transistors often are used as dummy transistors for other circuit parts and are sized to fit the layout best.
