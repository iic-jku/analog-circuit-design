...
As can be seen in the previous plot, an NMOS can be used to switch at potentials close to $\VSS$, while a PMOS can be used to switch at potentials close to $\VDD$. To construct a switch which can work for all voltage levels between $\VDD$ and $\VSS$ an NMOS is put in parallel to a PMOS, resulting in the well-known **transmission gate**.
