...
Since the gate is assumed at a fixed potential, we can put $\Cgs$ in parallel with $G_\mathrm{S}$ as $G_\mathrm{S}^{*} = G_\mathrm{S} + s \Cgs$, and we can put $\Cgd$ in parallel with $G_\mathrm{D}$ as $G_\mathrm{D}^{*} = G_\mathrm{D} + s \Cgd$. As a result, we will disregard these capacitors for now, and just consider $G_\mathrm{S}$ and $G_\mathrm{D}$, which leads to the simplified small-signal circuit shown in @fig-cascode-small-signal-simplified.
...
The output impedance of the lower MOSFET ($r_\mathrm{out} = 1 / \gds'$) is **increased** by the self-gain ($\gm / \gds$) of the cascode transistor! This is a powerful technique to increase the output impedance of a transistor stage by cascoding, much better than increasing $L$!
:::
