...
A popular way to stabilize a multi-pole feedback-system is to make one pole dominant, and try to shift the other poles to sufficiently high frequencies [@Mangelsdorf_2025_miller], that we have enough [phase margin](https://en.wikipedia.org/wiki/Phase_margin) in the closed-loop system. We may strive for $60^\circ$ as this will only cause a minor peaking in the frequency response [@Gray_Meyer_2009].
...
$$
A_\mathrm{cm,1} \approx -\frac{1}{\gm[3,5]} \cdot \frac{\gm[1,2] \gds[10]}{2\gm[1,2] + \gds[10]} \approx -\frac{\gds[10]}{2\gm[3,5]}.
$$ {#eq-diff-ota-cmgain1}
...
Combining @eq-diff-ota-cmgain1 and @eq-diff-ota-cmgain2 we arrive at (assuming that $\gm[3,5] \approx \gm[4,6]$)

$$
A_\mathrm{cm} = A_\mathrm{cm,1} \cdot A_\mathrm{cm,2} \approx \frac{\gds[10]}{2\gm[7,8]}
$$ {#eq-diff-ota-cmgain}
