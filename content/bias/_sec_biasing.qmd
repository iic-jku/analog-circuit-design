...
{{< include /content/bias/_fig_bias_current_gen.qmd >}}

The resistor $R_1$ will show variation with the process tolerance, but we can either (a) use an external resistor for a precise current generation, or (b) use some sort of trimming to correct this resistor value. Depending on the further use of the bias currents a variation of the resistor might not be a bad thing. Assuming we use the resulting bias current to generate somewhere a bias voltage $V_\mathrm{bias}$ by running the current through resistor $R_2$, then this voltage is given by
...
where $V_\mathrm{g0} = 1.205\,\text{V}$ is the bandgap voltage of silicon *extrapolated* to $0\,\text{K}$ and $\VBE[0]$ is the base-emitter voltage of a BJT at reference temperature $T_0$. (Note that this is not the physical bandgap of silicon at $0\,\text{K}$, which is about $1.17\,\text{eV}$; $V_\mathrm{g0}$ is the value obtained by extrapolating the nearly linear $\VBE(T)$ characteristic back to $T = 0\,\text{K}$.) Further, the difference in $\VBE$ of two BJT operated at different emitter current densities $J_1$ and $J_2$ is given as
...
Adding @eq-vbe-vs-temp and a scaled version of @eq-delta-vbe-vs-temp results in a reference voltage of value

$$
V_\mathrm{ref} =  V_\mathrm{g0} \left( 1 - \frac{T}{T_0} \right) + \VBE[0] \left( \frac{T}{T_0} \right) +  K \frac{k T}{q} \ln \left( \frac{J_1}{J_2} \right)
$$ {#eq-bandgap-voltage-with-temp}

where $K$ is a scaling factor that we are free to choose in the circuit implementation (we will see below that it is realized as a resistor ratio). Demanding zero temperature coefficient, $\partial V_\mathrm{ref} / \partial T = 0$, fixes it to

$$
K \frac{k}{q} \ln \left( \frac{J_1}{J_2} \right) = \frac{V_\mathrm{g0} - \VBE[0]}{T_0}.
$$ {#eq-bandgap-zero-tc}

Substituting @eq-bandgap-zero-tc back into @eq-bandgap-voltage-with-temp makes every term proportional to $T$ cancel, and only

$$
V_\mathrm{ref} =  V_\mathrm{g0}
$$ {#eq-bandgap-voltage}

remains. We thus have created an on-chip reference (called the [*bandgap voltage reference*](https://en.wikipedia.org/wiki/Bandgap_voltage_reference)) which is almost independent of manufacturing tolerances with a very small temperature coefficient. Of course, this is only true neglecting second-order effects, but nevertheless, reference accuracies of $\pm 1 \ldots 3\%$ without trimming are perfectly possible.

The original implementation in [@Widlar_1971] uses NPN transistors. The question is, where do we find BJT in a CMOS process? Luckily, when looking at the typical implementations, we find that there is a layer sandwich of P-N-P available. While the PNPs constructed parasitically out of these available layers are available for free without extra processing cost, they are very slow, show unusually small $\beta < 10$, and the collector is tied to $\VSS$ as it is the substrate. Still, for bandgap references, they are very useful.

A simple implementation of a bandgap reference circuit is shown in @fig-bandgap-simple [@Gray_Meyer_2009]. If we scale $R_1$ and $R_2$ correctly then we can achieve @eq-bandgap-voltage. Note that the output voltage is ca. $1.2\,\text{V}$, so operating this circuit on low supply voltages will be problematic.

{{< include /content/bias/_fig_bandgap_reference_simple.qmd >}}

$M_1$ to $M_4$ are scaled in a way that in both branches the same bias current is flowing. Further, $M_1$ and $M_2$ ensure that there is the same potential at their sources. Since the two PNP are scaled by the factor $m$, they are operated at different current densities despite carrying the same current, and consequently the following voltage develops across $R_1$:
...
Inspecting @eq-bandgap-bias-current we see that $I_\mathrm{bias} = k_1 T$ is a linear function of temperature $T$, a property that is very useful and called **PTAT** (proportional to absolute temperature). Strictly, this holds only insofar as $R_1$ itself is temperature-independent; a real resistor has a noticeable temperature coefficient (see @tbl-sg13g2-devices), which is one reason why the resistor type matters in a bandgap design. With $M_5$ we mirror this bias current into the output branch, and the output voltage $V_\mathrm{ref}$ is then given by

$$
V_\mathrm{ref} = \VBE + \frac{R_2}{R_1} \frac{k T}{q} \ln m.  
$$

Comparing with @eq-bandgap-voltage-with-temp we see that the scaling factor $K$ is realized here as the resistor ratio $R_2 / R_1$. By proper selection of $R_1$, $R_2$ and $m$ we can thus satisfy @eq-bandgap-zero-tc and arrive at @eq-bandgap-voltage.
...
Many more different bandgap architectures exist, with the Kuijk [@Kuijk_bandgap] or the Brokaw [@Brokaw_bandgap] types being popular choices.
:::
...
There is no circuitry added for enabling/disabling the circuit, which would also be needed for a practical implementation. As usual, the MOSFET sizing has been done in this [notebook](./gmid/sizing_bandgap_simple.ipynb).
...
The Xschem schematic is available [here](./xschem/bandgap_simple.sch) and the simulated reference voltage vs. temperature is shown in @fig-bandgap-sim-result. For a typical process we achieve a spread of the reference voltage of about $\pm 0.16\%$ over the simulated temperature range of $-30\,\text{degC}$ to $125\,\text{degC}$, which corresponds to an average temperature coefficient of ca. $20\,\text{ppm/K}$ (the ppm/K figure is the one you will usually find in a datasheet).
...
The CTAT and PTAT currents required for compensating each other's TCs are built using a $\Delta \VBE$ cell (given the PTAT current) with parallel resistors (providing the CTAT current). Voltages are sensed using an error amplifier and current sources are controlled to achieve matching currents in both branches. This current can then also be mirrored to flow through another resistor which can be scaled to produce the output voltage.
