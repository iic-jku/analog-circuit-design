...
where $V_\mathrm{g0} = 1.205\,\text{V}$ is the bandgap voltage of silicon at $0\,\text{K}$ and $\VBE[0]$ is the base-emitter voltage of a BJT at reference temperature $T_0$. Further, the difference in $\VBE$ of two BJT operated at different emitter current densities $J_1$ and $J_2$ is given as
...
$M_1$ to $M_4$ are scaled in a way that in both branches the same bias current is flowing. Further, $M_1$ and $M_2$ ensure that there is the same potential at their sources. Since the PNP are scaled by the factor $m$ (thus the current density is different) so that the following voltage develops across $R_1$:
