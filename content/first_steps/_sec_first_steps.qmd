...
::: {.callout-important title="MOSFET Symbol"}
Looking at the MOSFET symbols in @fig-nmos-symbol and @fig-pmos-symbol, you might have noticed that the left and the right symbols are symmetric for the drain and source terminals, while the middle symbol shows an arrow in the source connection.

This is a common "error" in many important textbooks and online resources [@Razavi_Analog_CMOS; @Gray_Meyer_2009; @Sansen_Essentials], and it is important to be aware of this. The drain and source of a MOSFET are (usually) symmetric, and the current flow can be reversed by changing the applied voltages (and then also drain and source swap; generally, for an NMOS, the connection with the higher potential is the drain, the one with the lower potential is the source. For a PMOS, the situation is inverted).

The symbol in the middle of @fig-nmos-symbol and @fig-pmos-symbol is thus misleading, because is suggests that the source is special and that the current can only flow in one direction. This is not the case, and it is important to be aware of this when designing circuits, especially when using MOSFETs as switches. As many symbols are drawn this way, we will also use this symbol in this course (and the resemblance to a bipolar junction transistor is intentional), but please be aware of the fact that the drain and source are symmetric, and the current can flow in both directions depending on the bias voltages!
:::
...
Now, in order to get started, basic Xschem testbenches are prepared, and first simple dc sweeps of various voltages and currents will be done. But before that, please look at the important note below!
...
and the output conductance $\gds$ models the dependence of the drain current on $\VDS$:
...
Like any electronic device, the MOSFET introduces noise into the circuit. In this course we will only consider the **drain-source current noise** of the MOSFET, given by
...
As a home exercise, try to derive @eq-mosfet-transit-frequency starting from @fig-mosfet-small-signal-model-simplified. This is a useful exercise to verify your understanding of the small-signal model.
::: 
...
3. Go back to your testbench for the LV PMOS `sg13_lv_pmos`:
