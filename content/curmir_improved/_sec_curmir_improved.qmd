...
Looking at the different current mirror architectures in @sec-current-mirror-improved, we can see that the current mirror matching is dominated by the bottom transistor pair (assuming equal $\VDS$), with the exception of the resistively degenerated current mirror. We have asserted that the matching of the resistively-degenerated current is improved due to the lower impact of MOSFET mismatch compared to resistor mismatch. We will now substantiate this claim.
...
$$
\sigma \left( \frac{\Delta \ID}{\ID} \right) \approx \sqrt{ \left( \frac{\sigma(\Delta \Vth)}{V_\mathrm{drv} + V_\mathrm{deg}} \right)^2 + \left[ \frac{V_\mathrm{drv}}{V_\mathrm{drv} + V_\mathrm{deg}} \cdot \sigma \left(\frac{\Delta K}{K}\right) \right]^2 + \left[ \frac{V_\mathrm{deg}}{V_\mathrm{drv} + V_\mathrm{deg}} \cdot \sigma \left( \frac{\Delta R}{R} \right) \right]^2}.
$$

The definition of $V_\mathrm{drv}$ is adapted here and understood as half of the $\VDS$ saturation voltage, i.e., $V_\mathrm{drv} = \VDS[,sat] / 2$. The degeneration voltage is the voltage drop across the degeneration resistors and given by $V_\mathrm{deg} = \ID \cdot R$.
...
The standard deviation of the transconductance parameter mismatch $\sigma(\Delta K / K)$ can be calculated similarly as (which is an alternative form to @eq-mosfet-mismatch-kp) as

$$
\sigma \left( \frac{\Delta K}{K} \right) = \sqrt{ \left[ \frac{\sigma (\Delta W)}{W} \right]^2 + \left[ \frac{\sigma (\Delta L)}{L} \right]^2 },
$$ {#eq-mosfet-mismatch-kp-alt}

where $\sigma (\Delta W)$ and $\sigma (\Delta L)$ are the process-dependent mismatch parameter for the width $W$ and length $L$, respectively.
...
| Component             | Standard deviation     | Value     |
|:----------------------|------------------------|-----------|
| Resistor `rsil`       | $\sigma(\Delta R / R)$ | 1.2 %µm   |
| Resistor `rppd`       | $\sigma(\Delta R / R)$ | 1.5 %µm   |
| Resistor `rhigh`      | $\sigma(\Delta R / R)$ | 5 %µm     |
| MOSFET `sg13_lv_nmos` | $\sigma(\Delta \Vth)$  | 2 mV µm   |
| MOSFET `sg13_lv_nmos` | $\sigma(\Delta W)$     | 4 nm      |
| MOSFET `sg13_lv_nmos` | $\sigma(\Delta L)$     | 2 nm      |
| MOSFET `sg13_lv_pmos` | $\sigma(\Delta \Vth)$  | 2.5 mV µm |
