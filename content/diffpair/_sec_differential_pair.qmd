...
3. For the typical case that $\gm \gg g_\mathrm{tail}$ then $I_\mathrm{out} \approx (g_\mathrm{tail}/2) V_\mathrm{in}$.

Note that @eq-differential-pair-cm is closely related to the general case of a degenerated common-source transistor stage (see @fig-common-source-degenerated). The effective transconductance $\gm'$ of a degenerated CS stage is given by

$$
\gm' = \frac{I_\mathrm{out}}{V_\mathrm{in}} = \frac{1}{\gm^{-1} + R_\mathrm{degen}}.
$$

For the differential-pair half circuit in common-mode operation, the effective source degeneration is $R_\mathrm{degen} = 2/g_\mathrm{tail}$, yielding @eq-differential-pair-cm.
